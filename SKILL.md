...
- 若宿主已提供 `harmonyos_developer_knowledge` MCP，优先用它检索当前鸿蒙官方开发指南、API 参考、版本说明和上架规范；搜索片段不足时，再按工具契约获取完整文档。
- 若该 MCP 不可用，视为用户未配置或已选择不配置。不要在普通开发任务中重复询问或擅自修改全局 MCP 配置；继续使用目标仓库、匹配版本的本地 SDK 和可访问的华为官方文档完成任务。
- 不因 MCP 可用而跳过版本核对、项目事实检查或本地构建验证。
